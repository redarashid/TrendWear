...
export default function RateCost({from='Home',name,stars=5,cost,discount=0}) {
  return (
    <div className={`${from==='Home'?'flex flex-col justify-between h-full':''}`}>
      <div>
        <Reavel>
          <h1 className={`font-bold ${from!='Home'?'bolded text-lg sm:text-4xl max-w-[600px]':'text-sm sm:text-xl'}`}>
            {name}
          </h1>
        </Reavel>
      </div>
      <div>
        ...
        <div className={`${from!='Home'?'mt-3':''}`}></div>
        <Reavel>
          <div className={`flex gap-1 ${from!='Home'?'text-lg sm:text-2xl gap-3':''}`}>
